...
def collate_fn(examples):
        ...
        gt_pixel_values = torch.stack([example["gt_pixel_values"] for example in examples])
        gt_pixel_values = gt_pixel_values.to(memory_format=torch.contiguous_format).float()
        input_ids = torch.stack([example["input_ids"] for example in examples])
        return {
            "pixel_values": pixel_values, 
            "input_ids": input_ids,     # NOTE: different from inferencing
...
class ReconCTDataset(Dataset):
    def __init__(self, file_paths, image_transforms=None, cond_transforms=None, tokenizer=None):
        ...
        self.id_map = pd.read_csv("splits/FELIX_OUT_ID_MAP.csv")
    ...
    def __getitem__(self, idx):
        ...
        bdmap_id = ct_path.split("/")[-2]
        ct_slice_raw, cond_ct_slice_raw, gt_slice_raw = load_CT_slice(os.path.join(ct_path, "ct.h5"))    # random slice

        ct_cond_transformed = self.image_transforms(
            image=ct_slice_raw, 
            cond=cond_ct_slice_raw, 
            mask=gt_slice_raw
        )
        ct_slice = ct_cond_transformed["image"]  
        cond_ct_slice = ct_cond_transformed["cond"]  
        gt_slice = ct_cond_transformed["mask"]  
        ...
        if "arterial" in self.id_map[self.id_map["BDMAP Name"]==bdmap_id]["Original Name"].item().lower():
            text_prompt = "An Arterial CT slice."
        elif "venous" in self.id_map[self.id_map["BDMAP Name"]==bdmap_id]["Original Name"].item().lower():
        ...
        example["gt_pixel_values"] = gt_slice
...
class CTDatasetInference(Dataset):    # for a single CT volume
    def __init__(self, file_path, image_transforms=None, cond_transforms=None):
        ...
        ### Deprecated
        self.image_transforms = image_transforms
        self.cond_transforms = cond_transforms  # NOTE: useless
    ...
    def __getitem__(self, slice_idx): # slice_idx will always in order by setting `shuffle=False`
        ...
        if "arterial" in self.id_map[self.id_map["BDMAP Name"]==self.bdmap_id]["Original Name"].item().lower():
            text_prompt = "An Arterial CT slice."
        elif "venous" in self.id_map[self.id_map["BDMAP Name"]==self.bdmap_id]["Original Name"].item().lower():
            text_prompt = "A Portal-venous CT slice."
        else:
            text_prompt = "An Arterial CT slice."   # default

        example = dict()
        # example["pixel_values"] = cond_ct_slice # NOTE: useless
        example["cond_pixel_values"] = cond_ct_slice
